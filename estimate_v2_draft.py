...
STEP 3B — IF SUB (electrical / major HVAC / major plumbing / crawlspace):
  ...
  WRITING "skipped_items" — every entry MUST be written as
  "item name - reason", with both halves present and separated by " - ".
  The reason half is written verbatim into the estimate in JobTread for
  OCC's team to read when they review it, so it has to be a real
  explanation someone can act on, not a restatement of the item name.
    Good: "Water heater replacement - requires an on-site sub quote; the
          report doesn't state tank size or venting type"
    Good: "Radon mitigation - not a service OCC offers"
    Bad:  "Water heater - skipped"     (says nothing)
    Bad:  "Radon mitigation"           (no reason at all)
...
EXAMPLE_OUTPUT_SCHEMA = """
{
  ...
  "total": 0.00,
  "skipped_items": ["Water heater replacement - requires an on-site sub quote; the report doesn't state tank size or venting type"],
  "needs_consult": false,
  "consult_reason": ""
}
"""
...
def _build_skipped_items_note(skipped_items):
    ...
    for raw in skipped_items or []:
        item_name, _reason = _split_skipped_item(raw)
        if item_name:
...
def _split_skipped_item(raw):
    """Split one raw "skipped_items" entry into (item_name, reason).

    Claude is asked to write these as "item - reason", but real output
    drifts: it sometimes uses an em/en dash, sometimes a colon, and
    occasionally gives no reason at all. This normalizes all of those into
    the same two-part shape so the reason can be surfaced instead of
    thrown away. Returns ("", "") for an empty entry.

    Splits on the FIRST separator only — reasons routinely contain their
    own dashes ("requires an on-site sub quote — insufficient detail"),
    and splitting on all of them would truncate the explanation to its
    first clause.
    """
    raw = (raw or "").strip()
    if not raw:
        return "", ""
    # Longest/most specific separators first so " — " isn't matched by "-".
    for sep in (" - ", " — ", " – ", " -- ", ": "):
        if sep in raw:
            name, reason = raw.split(sep, 1)
            return name.strip(), reason.strip()
    return raw, ""


def _build_skipped_item_reason(reason):
    """The description text for one skipped item's $0 line item.

    Jason's request (Aug 2026, reviewing 102 Tuscany Way): the "Not
    Included In This Estimate" group listed WHAT was left out but not WHY,
    so the reasoning existed only in Claude's raw JSON in the Render logs.
    "Gives me context without having to go looking for it."

    When Claude gives no reason, say so plainly rather than leaving the
    description empty — a blank description reads as "no reason needed",
    which is the opposite of what an empty reason actually means.
    """
    reason = (reason or "").strip()
    if not reason:
        return ("No reason was given for leaving this out — worth a look "
                "before the estimate goes out.")
    # Capitalize the first letter for readability; Claude's reasons usually
    # start lowercase ("requires an on-site sub quote...").
...
def add_cost_groups_v2(job_id, estimate, jobtread_query_fn, org_id=None,
                ...
                jobtread_query_fn({
                    "createCostItem": {
                        "$": {
                            ...
                            # Aug 2026: this item's name has always CLAIMED
                            # it isn't shown to clients, but it was relying
                            # entirely on the estimate template not
                            # rendering line items. costItem descriptions
                            # default to showDescription: true, so pin it
                            # off explicitly — the confidence flags and
                            # quantity assumptions in here are review notes
                            # for OCC, not client-facing scope.
    ...
    #
    # Aug 2026 (Jason, reviewing 102 Tuscany Way): the group listed the
    # item names but none of the reasoning, so "why isn't this in here?"
    # meant digging through Render logs for the raw JSON. Each skipped item
    # now also gets its own $0 cost item inside the group — name = the
    # item, description = Claude's actual reason for leaving it out.
    #
    # Same safety argument as the per-group "NOTES (internal)" item above:
    # Jason's client-facing estimate template renders group names and
    # descriptions but never the line items beneath them (confirmed Jul
    # 2026), so the client still sees only the neutral one-paragraph note
    # in the group description, while his team gets the reasoning sitting
    # right there in the budget. The two audiences stay separated by the
    # template, not by hoping nobody scrolls.
    skipped_raw = estimate.get("skipped_items") or []
    skipped_note = _build_skipped_items_note(skipped_raw)
    if skipped_note:
        item_count = skipped_note.count("\n- ")
        skipped_group_id = None
        try:
            resp = jobtread_query_fn({
                "createCostGroup": {
            ...
            skipped_group_id = (resp or {}).get("createCostGroup", {}).get("createdCostGroup", {}).get("id")
        ...
        # One $0 line item per skipped item, carrying the reasoning. Each
        # is independently non-fatal — a single malformed entry shouldn't
        # cost the others their explanation, and none of it should ever
        # take down an estimate that's already written.
        if skipped_group_id:
            reasons_added = 0
            for raw in skipped_raw:
                item_name, reason = _split_skipped_item(raw)
                if not item_name:
                    continue
                try:
                    jobtread_query_fn({
                        "createCostItem": {
                            "$": {
                                "costGroupId": skipped_group_id,
                                "name": item_name[:100],
                                "quantity": 1, "unitCost": 0, "unitPrice": 0,
                                "description": _build_skipped_item_reason(reason),
                                # costItem descriptions default to
                                # showDescription: true — they DO print on
                                # customer-facing documents. This reasoning
                                # is internal ("not a service OCC offers",
                                # "requires an on-site sub quote"), so it's
                                # pinned off explicitly rather than relying
                                # on the estimate template not rendering
                                # line items.
                                "showDescription": False,
                                "costCodeId": COST_CODE_UNCATEGORIZED,
                    ...
                    print(f"  Skipped-item reason line failed for '{item_name[:50]}' (non-fatal): {e}")
            print(f"  Added {reasons_added}/{len(skipped_raw)} skipped-item reason line(s)")
...
# ─────────────────────────────────────────────────────────────────────────
# ATTACHMENT TYPE SNIFFING (Aug 2026 — Meredith Tye / 111 Paxton St failure)
# ─────────────────────────────────────────────────────────────────────────
# BUG FIX #4: a submission came in where the "Repair Addendum" was a 13,177-
# byte file starting with b'PK\x03\x04\x14' — a ZIP header, i.e. a Word .docx,
# not a PDF. The realtor typed the addendum in Word and uploaded the .docx.
#
# Nothing in the pipeline checked. The bytes were base64'd and sent with
# media_type "application/pdf", Anthropic rejected the request with HTTP 400,
# and the retry loop tried the exact same malformed payload twice more before
# giving up. The lead went to manual entry.
#
# The galling part: the inspection report in that same submission was a
# perfectly good 6.7MB PDF. One unreadable attachment threw away a completely
# usable estimate. So there are two bugs here, and the second is worse:
#
#   1. The code assumed every download was a PDF.
#   2. A single bad attachment was fatal to the whole estimate.
#
# Fixed by sniffing the real type from magic bytes and converting to whatever
# Claude can actually read:
#   - PDF   -> native document block, unchanged (photos + markups intact)
#   - DOCX  -> text extracted from word/document.xml with the stdlib zipfile
#              module. No new dependency, no meaningful memory cost. A typed
#              addendum is pure text anyway, so nothing is lost.
#   - image -> native image block. Realtors do photograph addendums.
#   - other -> skipped, with the prompt told explicitly which document
#              couldn't be read, so Claude flags it instead of silently
#              pricing an incomplete scope.
# An unreadable addendum now degrades to "estimate from the inspection report
# and note the gap" instead of failing the job.

def _sniff_file_kind(data):
    """Identify a downloaded attachment from its magic bytes.

    Returns one of: 'pdf', 'docx', 'zip', 'jpeg', 'png', 'gif', 'webp',
    'rtf', 'doc', 'empty', 'unknown'. Never raises.

    Deliberately checks the actual bytes rather than trusting the file name
    or the Wufoo field it arrived in — the field said "Repair Addendum" and
    the extension may well have said .pdf; the bytes are the only thing that
    told the truth.
    """
    if not data:
        return "empty"
    ...
    if head.startswith(b"PK\x03\x04"):
        # ZIP container. Office Open XML (.docx/.xlsx/.pptx) all look like
        # this — look inside to tell them apart.
        try:
            import zipfile
    ...
    if head.startswith(b"\xd0\xcf\x11\xe0"):
        return "doc"        # legacy OLE2 .doc/.xls
    return "unknown"
...
def _docx_to_text(data, max_chars=200000):
    """Extract readable text from a .docx using only the standard library.

    A .docx is a ZIP whose word/document.xml holds the body. Paragraph and
    break tags are converted to real line breaks BEFORE tags are stripped —
    otherwise every line of the addendum runs together into one wall of text
    and the item-by-item structure that makes an addendum useful is lost.

    Returns "" if anything goes wrong; the caller treats that as unreadable.
    """
    try:
        import zipfile
        ...
        with zipfile.ZipFile(io.BytesIO(data)) as z:
            ...
            # Text boxes and shapes live in separate parts; pull them too so a
            # addendum laid out in a table/textbox isn't silently half-read.
            for extra in ("word/header1.xml", "word/footer1.xml"):
                if extra in names:
                    try:
                        xml += z.read(extra).decode("utf-8", errors="replace")
                    except Exception:
                        pass
        ...
        # Collapse the run of blank lines Word leaves behind, keep paragraphs.
        text = re.sub(r"[ \t]+\n", "\n", text)
        text = re.sub(r"\n{3,}", "\n\n", text)
        text = text.strip()
...
def prepare_attachment(data, label):
    """Turn raw downloaded bytes into something Claude can actually read.

    Returns (kind, payload):
      ('pdf',   bytes)  — hand to the existing native-PDF path
      ('text',  str)    — send as a text block
      ('image', (media_type, bytes)) — send as an image block
      ('none',  reason_string)       — unreadable; tell Claude it's missing

    Never raises. An attachment problem should degrade the estimate, not
    end it.
    """
    kind = _sniff_file_kind(data)
    ...
    if kind == "docx":
        ...
        if text:
            print(f"  {label} is a Word .docx ({len(data):,} bytes) — extracted "
                  f"{len(text):,} chars of text and sending as text")
            return "text", text
        return "none", f"{label} was a Word document that could not be read"

    if kind in _IMAGE_MEDIA_TYPES:
        print(f"  {label} is a {kind.upper()} image ({len(data):,} bytes) — "
              f"sending as a native image")
        return "image", (_IMAGE_MEDIA_TYPES[kind], data)

    if kind == "empty":
        return "none", f"{label} downloaded as an empty file"

    friendly = {
        "zip":     "a ZIP archive (possibly an Excel or PowerPoint file)",
        "doc":     "a legacy Word .doc file (pre-2007 format)",
        "rtf":     "an RTF document",
        "unknown": "a file in an unrecognized format",
    }.get(kind, f"an unsupported file ({kind})")
    print(f"  {label} is {friendly}, not a PDF ({len(data):,} bytes) — skipping it")
    return "none", f"{label} was {friendly} and could not be read"
...
def build_claude_document_content(addendum_pdf_bytes, inspection_pdf_bytes,
                                   ...
                                   addendum_blocks=None, inspection_blocks=None,
                                   unreadable=None):
    ...
    """
    # addendum_blocks / inspection_blocks (Aug 2026): already-built content
    # blocks for an attachment that wasn't a PDF — extracted .docx text, or a
    # photographed addendum as an image. They stand in for the document block.
    # unreadable: human-readable reasons for attachments we had to skip, so
    # the prompt can tell Claude what's missing rather than letting it price a
    # scope it never actually saw.
    addendum_blocks = addendum_blocks or []
    inspection_blocks = inspection_blocks or []
    unreadable = unreadable or []

    have_addendum = bool(addendum_pdf_bytes) or bool(addendum_file_id) or bool(addendum_blocks)
    have_inspection = bool(inspection_pdf_bytes) or bool(inspection_file_id) or bool(inspection_blocks)

    if have_addendum and have_inspection:
        doc_instructions = (
            "Two documents are provided below: the repair addendum first, then "
            "the inspection report. Process the repair addendum first to identify "
    ...
    if unreadable:
        doc_instructions += (
            "\n\nIMPORTANT — one or more uploaded files could not be read: "
            + "; ".join(unreadable)
            + ". Work from whatever IS provided below. Do NOT invent scope for the "
            "unreadable document. If the missing file was the repair addendum and "
            "you therefore cannot confirm which items the buyer actually requested, "
            "say so plainly in consult_reason and set needs_consult=true, and note "
            "in skipped_items that the addendum could not be read."
        )
...
"""
    ...
    if have_addendum:
        content.append({"type": "text", "text": "\n=== REPAIR ADDENDUM (read text, photos, and handwritten annotations; may be a scan) ==="})
        if addendum_blocks:
            content.extend(addendum_blocks)
        else:
            ...
            content.append(_pdf_document_block(
                ...
                # cache breakpoint goes on the LAST document block
                cache=not (have_inspection and not inspection_blocks),
            ))

    if have_inspection:
        content.append({"type": "text", "text": "\n=== INSPECTION REPORT (read text, photos, and handwritten annotations) ==="})
        if inspection_blocks:
            content.extend(inspection_blocks)
        else:
...
def call_claude_v2(addendum_pdf_bytes, inspection_pdf_bytes, client_name,
    ...
    #
    # This MUST run on the raw downloaded bytes, before the type routing
    # below. Routing rewrites a non-PDF into b"" plus content blocks, so a
    # dedupe placed after it would compare b"" to b"" and never fire — two
    # identical .docx uploads would both be sent. (Caught by the test suite
    # after the routing step was added, not in production.)
    #
    ...
    # ── Attachment type check (BUG FIX #4) ─────────────────────────────────
    # Run BEFORE the dedupe and Files API steps: everything downstream assumes
    # it's holding a PDF, and on the Meredith Tye submission it wasn't — the
    # addendum was a Word .docx that got labelled application/pdf and earned a
    # hard 400 from the API on all three attempts. Whatever isn't a PDF gets
    # converted to something Claude can read, or dropped with a reason.
    addendum_blocks = []
    inspection_blocks = []
    unreadable = []

    def _route(data, label):
        """Returns pdf_bytes to keep on the native path, plus appends to the
        blocks/unreadable lists as needed."""
        if not data:
            return b"", []
        kind, payload = prepare_attachment(data, label)
        if kind == "pdf":
            return payload, []
        if kind == "text":
            return b"", [{
                "type": "text",
                "text": (f"[{label} — original file was a Word document; its text is "
                         f"reproduced below verbatim]\n\n{payload}")
            }]
        if kind == "image":
            media_type, raw = payload
            return b"", [{
                "type": "image",
                "source": {
                    "type": "base64",
    ...
    addendum_pdf_bytes, addendum_blocks = _route(addendum_pdf_bytes, "Repair addendum")
    inspection_pdf_bytes, inspection_blocks = _route(inspection_pdf_bytes, "Inspection report")

    if not addendum_pdf_bytes and not inspection_pdf_bytes \
            and not addendum_blocks and not inspection_blocks:
        raise NonRetryableEstimateError(
            "No readable documents were provided — " + "; ".join(unreadable)
            + ". Retrying will not help; this lead needs manual entry."
        )
    ...
    try:
        ...
        content = build_claude_document_content(
            ...
            addendum_blocks=addendum_blocks,
            inspection_blocks=inspection_blocks,
